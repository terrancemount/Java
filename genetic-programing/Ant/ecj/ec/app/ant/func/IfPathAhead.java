...
public class IfPathAhead extends GPNode implements EvalPrint
    {
    public String toString() { return "if-path-ahead"; }
    ...
    public void eval(final EvolutionState state,
        ...
        switch (p.orientation)
            {
            case Ant.O_UP:
                if (p.map[p.posx][(p.posy-1+p.maxy)%p.maxy]==Ant.TRAIL)
                    children[0].eval(state,thread,input,stack,individual,problem);
                else children[1].eval(state,thread,input,stack,individual,problem);
                break;
            case Ant.O_LEFT:
                if (p.map[(p.posx-1+p.maxx)%p.maxx][p.posy]==Ant.TRAIL)
                    children[0].eval(state,thread,input,stack,individual,problem);
                else children[1].eval(state,thread,input,stack,individual,problem);
                break;
            case Ant.O_DOWN:
                if (p.map[p.posx][(p.posy+1)%p.maxy]==Ant.TRAIL)
                    children[0].eval(state,thread,input,stack,individual,problem);
                else children[1].eval(state,thread,input,stack,individual,problem);
                break;
            case Ant.O_RIGHT:
                if (p.map[(p.posx+1)%p.maxx][p.posy]==Ant.TRAIL)
                    children[0].eval(state,thread,input,stack,individual,problem);
                else children[1].eval(state,thread,input,stack,individual,problem);
                break;
            default:  // whoa!
